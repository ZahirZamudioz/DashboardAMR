...
def cargar_puntos_corte_clsi(ruta_clsi: str) -> Tuple[Dict, Dict]:
    ...
    #global puntos_corte_clasico, puntos_corte_alterno  # Declarar como globales
    ...
    try:
        ...
        print(puntos_corte_clasico)
        print(puntos_corte_alterno)
...
def obtener_puntos_corte(antibiotico: str, especie: str, metodo: str = 'clasico', clsi_df: pd.DataFrame = None,
    ...
    for (ab, grupo, modo_exc, excluidas), valores in diccionario.items():
        if ab == antibiotico and modo_exc and especie not in excluidas:
            ...
            if especie in clsi_df.loc[clsi_df['Grupo_general'] == grupo, 'Grupo/Especie especifica'].values or grupo == especie:
                return valores"""
            # El grupo general debe coincidir
...
def categorizar_dataframe(data: pd.DataFrame, puntos_corte_clasico: Dict, puntos_corte_alterno: Dict) -> pd.DataFrame:
    ...
    print(f"Columnas de antibióticos a categorizar: {columnas_ab}")
...
if __name__ == "__main__":
    data_final, diccionarios_finales, clsi_df, puntos_corte_clasico, puntos_corte_alterno = main()
    print(data_final)
    data_final.to_excel('/Users/zahir/Downloads/PruebaLimpiezaNuevoV4.xlsx', index=False)'''
